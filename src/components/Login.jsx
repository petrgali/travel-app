...
const Login = (props) => {
  ...
  const handleLogin = async () => {
    ...
    if (response.message || response.error) {
      setService({ ...service, isLoading: false, error: (response.message || response.error) })
    } else {
    ...
    <Dialog fullWidth maxWidth="xs" open={isOpen} onClose={handleClose}>
      ...
      <DialogContent className={classes.dialogContent} dividers>
        <form className={classes.form}>
          {service.error !== null && (
            <MuiAlert
              onClose={() => setService({ ...service, error: null })}
              severity="error"
            >
              {service.error}
            </MuiAlert>
          ...
          <Button
            ...
            disabled={!canLogin()}
          >
            {service.isLoading && <CircularProgress size={24} color="white" />}
